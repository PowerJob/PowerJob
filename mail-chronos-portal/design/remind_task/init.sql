...
CREATE TABLE `sx_sp_rt_task_instance` (
                                          ...
                                          `custom_id` bigint NOT NULL COMMENT '业务方定义的 ID',
                                          ...
                                          KEY `idx1_sx_sp_rt_task_instance` (`expected_trigger_time`,`enable`),
                                          KEY `idx2_sx_sp_rt_task_instance` (`task_id`),
                                          KEY `idx3_sx_sp_rt_task_instance` (`custom_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='提醒任务实例,rt means remind task'
    PARTITION BY RANGE (partition_key)(PARTITION p0 VALUES LESS THAN (20211028) ENGINE = InnoDB,
        ...
        PARTITION p20211030 VALUES LESS THAN (20211031) ENGINE = InnoDB);

alter TABLE `sx_sp_rt_task_instance` add PARTITION(
    PARTITION p20211131 VALUES LESS THAN (20211101) ENGINE = InnoDB
    );

select partition_name part,partition_expression expr,partition_description descr,table_rows from information_schema.partitions  where table_schema = schema() and table_name='sx_sp_rt_task_instance';
